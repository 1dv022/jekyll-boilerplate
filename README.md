...
4. Install bundler with `gem install bundler`. Then install dependencies and github-pages `npm install` (if you are on Windows use the command `npm install --no-bin-links`). It will now install some packages and it could take a while.
